import sys
sys.path.append(r'C:\Users\PyPit\OneDrive\Desktop\cheats')
import dxshot as dxcam
import torch
import cv2
import numpy as np
import time
...
model = torch.hub.load(r'C:\Users\PyPit\OneDrive\Desktop\cheats\yolov5' , 'custom', path= r'C:\Users\PyPit\OneDrive\Desktop\cheats\best_new.engine',source='local').cuda()
model.conf = 0.25
model.maxdet = 10
model.amp = True 
model.classes = [1]
...
while True:
    ...
    if closest_part != -1:
        ...
        if silent_aim == True and silent_aim_not_cooldown[0] == True:
            ...
            thread.start()
        if aim_assist == True and closest_part_distance < activation_range and send_next[0] == True and win32api.GetAsyncKeyState(0x01)&0x8000 > 0:
            xdif = (head_center_list[0] - screenshot_center[0])*AIM_SPEED*target_multiply[MONITOR_SCALE]
            ydif = (head_center_list[1] - screenshot_center[1])*AIM_SPEED*target_multiply[MONITOR_SCALE]
            if abs(xdif) > 20:
                xdif *= 0.2
